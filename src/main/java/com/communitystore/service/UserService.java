...
@Service public class UserService {
    ...
    public UserService(UserRepository users, PasswordEncoder passwordEncoder) { this.users = users; this.passwordEncoder = passwordEncoder; }
    ...
    @Transactional(readOnly = true) public User findByEmail(String email) { return users.findByEmailIgnoreCase(email).orElse(null); }
    @Transactional(readOnly = true) public List<User> findAll() { return users.findAll(); }
    @Transactional public User verifyVendor(Long id) { User user = users.findById(id).orElseThrow(() -> new IllegalArgumentException("User not found")); if (user.getUserType() != UserType.VENDOR) throw new IllegalArgumentException("Only vendors require verification"); user.setVerified(true); user.setAccountStatus(UserStatus.ACTIVE); return users.save(user); }
    @Transactional public User updateStatus(Long id, UserStatus status) { User user = users.findById(id).orElseThrow(() -> new IllegalArgumentException("User not found")); user.setAccountStatus(status); return users.save(user); }
}
