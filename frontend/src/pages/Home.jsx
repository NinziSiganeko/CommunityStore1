    ...
    <div className="scroll-area">
      <div className="hero">
        ...
        <h2 className="hero-greeting">
          Hello, {greetingName} <span className="role">{roleLabel}</span>
        </h2>
        <button className="search-bar search-bar-button" onClick={() => navigate("/marketplace")} aria-label="Open marketplace search">
          <i className="bi bi-search" />
          <span>Search textbooks, electronics, clothing...</span>
        </button>
      </div>
      ...
      <div className="categories-section">
        <div className="categories-grid">
          {CATEGORIES.map((cat) => (
              <button key={cat.label} className="category-btn" style={{ background: cat.bg }} onClick={() => navigate("/marketplace")}>
                {cat.icon}
      ...
      <div className="trending-section">
        <div className="section-header">
          <h3 className="section-title">Trending in Community</h3>
          <button className="see-all-btn" onClick={() => navigate("/marketplace")}>SEE ALL</button>
        </div>
        <div className="trending-scroll">
          {TRENDING_ITEMS.map((item) => (
              <div key={item.id} className="trending-card" onClick={() => navigate("/marketplace")} role="link" tabIndex="0">
                <div className="trending-card-img-wrap">
                  <img src={item.img} alt={item.name} />
                  <span className="badge-img-overlay"><Badge type={item.badge} /></span>
                </div>
      ...
      <div className="announcements-section">
        <h3 className="section-title" style={{ marginBottom: 14 }}>Community Announcements</h3>
        {ANNOUNCEMENTS.map((a) => (
